package Evaluacion_integradora;
...
public class Evaluacion_integradora {
	...
	public static void analizarDatosTresJugadores() {
		...
		while (mod_camisa == false) {
			...
			try {
				String ruta = "C:\\Users\\Alumno.A02-M19\\Documents\\ListaJugadores\\archivo_jugadores.txt"; //Poner la ruta de la carpeta

				File file = new File(ruta);

				
				if (!file.exists()) {
					file.createNewFile(); // crea un archivo si no existe uno
				}
				...
				for (int i = 0; i < 3; i++) {
					bw.write("Nombre: " + nombre[i] + ", Apellido: " + apellido[i] + ", Numero de camisa: " + nro_camisa[i] );
		...
		while (mod_camisa == false) {
			...
			try {
				...
				for (int i = 0; i < 4; i++) {
					...
					bw.write("Nombre: " + nombre_supl[i] + ", Apellido: " + apellido_supl[i] + ", Numero camisa: " + nro_camisa_supl[i]);
